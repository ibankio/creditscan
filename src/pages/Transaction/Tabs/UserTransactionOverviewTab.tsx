...
const ECONIA_MARKETS = [
  {
    ...
    base_module_name: "creditchain_coin",
    base_struct_name: "CreditChainCoin",
  ...
  },
  {
    ...
    quote_module_name: "creditchain_coin",
    quote_struct_name: "CreditChainCoin",
    tick_size: 1,
    lot_size: 1000000,
  },
  {
...
function parseAmisLSDEvent(event: Types.Event): LiquidStaking | undefined {
  ...
  const creditchain_coin = "0x1::creditchain_coin::CreditChainCoin";
  const liquid_aptos =
  ...
  if (subActionType === "mint") {
    ...
    assetData.push({asset: creditchain_coin, amount: Number(data.apt)});
    ...
    assetData.push({asset: creditchain_coin, amount: Number(data.amount)});
...
function parseThalaLSDEvent(event: Types.Event): LiquidStaking | undefined {
  ...
  if (subActionType === "mint") {
    ...
    assetData.push({asset: creditchain_coin, amount: Number(data.staked_APT)});
  } else if (subActionType === "stake") {
    const data: {
      thAPT_staked: string;
    ...
    assetData.push({
      asset: creditchain_coin,
      amount: Number(data.actual_amount),
    });
  } else if (subActionType === "withdraw") {
    const data: {
    ...
    assetData.push({
      asset: creditchain_coin,
      amount: Number(data.unstaked_amount),
    });
  }
...
function parseKofiLSDEvent(event: Types.Event): LiquidStaking | undefined {
  ...
  if (subActionType === "mint") {
    ...
    assetData.push({asset: liquid_aptos, amount: Number(data.kapt_amount)});
    assetData.push({asset: creditchain_coin, amount: Number(data.apt_amount)});
  } else if (subActionType === "withdraw") {
    const data: {
      apt_amount: string;
    } = event.data;
